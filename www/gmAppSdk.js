(function () {
    const getSafeParentGm = () => {
        try {
            const parentGm = window.parent && window.parent !== window ? window.parent.$gm : null;
            return parentGm && typeof parentGm === 'object' ? parentGm : {};
        } catch (error) {
            return {};
        }
    };

    const ensureGmDefaults = (source = {}) => ({
        version: '',
        changelog: '',
    ...
    // 初始化 $gm 对象
    window.$gm = ensureGmDefaults({
        ...getSafeParentGm(),
        ...(window.$gm && typeof window.$gm === 'object' ? window.$gm : {}),
        ...getURLParams(window.location.href),
    });

    try {
        if (window.parent && window.parent !== window) {
            window.parent.$gm = ensureGmDefaults(window.parent.$gm);
        }
    } catch (error) {
        // Ignore parent context assignment failures
    }
    // 消息类型处理器映射
